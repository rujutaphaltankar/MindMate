...
const quickActions = [
  {
    ...
    gradient: "from-dusk-500/25 to-dusk-600/15",
  ...
  },
  {
    ...
    gradient: "from-sage-500/25 to-sage-600/15",
  ...
  },
  {
    ...
    gradient: "from-purple-500/25 to-purple-700/15",
  ...
  },
  {
    ...
    gradient: "from-sand-300/20 to-sand-400/10",
...
const statCards = [
  {
    ...
    label: "Current Mood",
    unit: "/10",
    gradient: "from-dusk-500/20 to-dusk-700/10",
    border: "border-dusk-500/20",
    dot: "bg-dusk-400",
    textColor: "text-dusk-200",
  },
  {
    key: "stress",
    label: "Stress Level",
  ...
  },
  {
    ...
    unit: "h",
    gradient: "from-cyan-500/15 to-cyan-700/8",
  ...
  return (
    <AppShell>
      ...
      <div className="mb-8 animate-fade-in-up">
        <p className="text-xs font-semibold uppercase tracking-widest text-dusk-500 mb-1">
          {greeting()}
        </p>
        <h1 className="font-display text-3xl text-white">
          {user?.name ? `${user.name}.` : "Welcome."}
        </h1>
        <p className="mt-1 text-sm text-dusk-400">Here's a snapshot of how you're doing.</p>
      </div>
        ...
        {statCards.map((card, i) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-xs font-medium text-dusk-400 uppercase tracking-wider">{card.label}</p>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {quickActions.map((a, i) => (
            ...
            >
              ...
              <span className="text-xs font-medium text-dusk-300">{a.label}</span>
            </Link>
          ))}
        </div>
      </div>
      ...
      <div className="mt-8 card-glass p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-display text-lg text-white">Wellness snapshot</h2>
          <Link to="/insights" className="text-xs text-sage-400 hover:text-sage-300 transition-colors font-medium">
            Full insights →
          </Link>
        </div>
        ...
        {records.length === 0 ? (
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            {[
              { label: "Avg mood", value: avgMood, color: "text-dusk-200" },
              { label: "Avg stress", value: avgStress, color: "text-amber-200" },
              { label: "Avg energy", value: avgEnergy, color: "text-sage-200" },
            ].map((item) => (
              <div key={item.label} className="py-3 rounded-2xl bg-white/3 border border-white/6">
                <p className={`font-display text-2xl font-semibold ${item.color}`}>{item.value ?? "—"}</p>
                <p className="text-xs text-dusk-500 mt-1">{item.label}</p>
              </div>
            ))}
          </div>
        )}
